...
const AIInsights = ({ heatmapData, isVisible }) => {
  ...
  const performLocalAnalysis = (data) => {
    ...
    // Data quality warning only (removed success message)
    if (totalPoints < 100) {
      generatedInsights.push({
        type: 'warning',
        title: 'Limited Data Sample',
        description: `Only ${totalPoints} gaze points collected.`,
        recommendation: 'Collect more data (aim for 500+ points) for more reliable insights. Track for a longer duration.',
        data: { totalPoints }
      });
    }
      ...
      {!showPanel ? (
        ...
        <div style={{
          ...
          {isAnalyzing ? (
            ...
            <p style={{ color: '#64748b', fontSize: '14px', textAlign: 'center' }}>
              Click "Show Heatmap" to generate insights.
            </p>
          )}
